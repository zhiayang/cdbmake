...
namespace zprocpipe
{
	...
	public:
		...
		{
			return readline_impl(m_stdout_pipe, m_stdout_buffer);
		}

		std::string readStderrLine()
		{
			return readline_impl(m_stderr_pipe, m_stderr_buffer);
		}
		...
		const std::filesystem::path& cwd)
	{
		...
		if(auto child = fork(); child < 0)
		{
			return {
				std::nullopt,
				std::string("fork(): ") + os::strerror_wrapper()
			};
		}
		else if(child == 0)
		{
