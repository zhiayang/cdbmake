...
namespace zst
{
	namespace impl
	{
		...
		struct str_view
		{
			...
			template <typename T, typename = typename detail::enable_if<detail::is_same<char, value_type>::value &&
																		detail::is_same<const char*, T>::value>::type>
			...
			constexpr inline bool operator==(const str_view& other) const
			{
				return (this->len == other.len) &&
				       (this->ptr == other.ptr ||
						   (memcmp(this->ptr, other.ptr, detail::min(this->len, other.len)) == 0));
			}
			...
			constexpr inline const value_type* begin() const
			{
				return this->ptr;
			}
			constexpr inline const value_type* end() const
			{
				return this->ptr + len;
			}
			...
			constexpr inline const value_type* data() const
			{
				return this->ptr;
			}
			...
			inline size_t find(value_type c) const
			{
				return this->find(str_view(&c, 1));
			}
			...
			inline size_t rfind(value_type c) const
			{
				return this->rfind(str_view(&c, 1));
			}
			...
			{
				return str_view(
					this->ptr + pos, cnt == static_cast<size_t>(-1) ? (this->len > pos ? this->len - pos : 0) : cnt);
			}
			...
			inline bool ends_with(str_view sv) const
			{
				return this->take_last(sv.size()) == sv;
			}

			inline bool starts_with(value_type c) const
			{
				return this->size() > 0 && this->take(1)[0] == c;
			}
			inline bool ends_with(value_type c) const
			{
				return this->size() > 0 && this->take_last(1)[0] == c;
			}

			inline str_view& remove_prefix(size_t n)
			{
				return (*this = this->drop(n));
			}
			inline str_view& remove_suffix(size_t n)
			{
				return (*this = this->drop_last(n));
			}
			...
			template <typename T = value_type,
				typename = typename detail::enable_if<detail::is_same<T, char>::value>::type>
			...
			template <typename A = value_type,
				typename = typename detail::enable_if<detail::is_same<A, uint8_t>::value>::type>
			...
			{
				static_assert(sizeof(value_type) % sizeof(U) == 0 || sizeof(U) % sizeof(value_type) == 0,
					"types are not castable");
...
#if ZST_USE_STD
		template <typename C>
		inline bool operator==(const std::basic_string<C>& other, str_view<C> sv)
		{
			return sv == str_view<C>(other);
		}

		template <typename C>
		inline bool operator==(str_view<C> sv, const std::basic_string<C>& other)
		{
			return sv == str_view<C>(other);
		}

		template <typename C>
		inline bool operator==(const std::basic_string_view<C>& other, str_view<C> sv)
		{
			return sv == str_view<C>(other);
		}

		template <typename C>
		inline bool operator==(str_view<C> sv, const std::basic_string_view<C>& other)
		{
			return sv == str_view<C>(other);
		}
		...
		template <typename C>
		inline bool operator!=(std::basic_string_view<C> other, str_view<C> sv)
		{
			return sv != str_view<C>(other);
		}

		template <typename C>
		inline bool operator!=(str_view<C> sv, std::basic_string_view<C> other)
		{
			return sv != str_view<C>(other);
		}
#endif // ZST_USE_STD
		...
		template <typename C>
		inline const C* begin(const str_view<C>& sv)
		{
			return sv.begin();
		}
		template <typename C>
		inline const C* end(const str_view<C>& sv)
		{
			return sv.end();
		}
	}
...
namespace zst
{
	namespace impl
	{
		...
		struct buffer
		{
			...
			{
				return this->len == b.len && memcmp(this->mem, b.mem, sizeof(value_type) * this->len) == 0;
			}
			...
			{
				return byte_span(reinterpret_cast<const uint8_t*>(this->mem), this->len * sizeof(value_type));
			}
			...
			template <typename T, typename A = value_type,
				typename = typename detail::enable_if<detail::is_same<A, uint8_t>::value>::type>
	...
	template <typename C>
	inline const C* begin(const impl::buffer<C>& sv)
	{
		return sv.begin();
	}
	template <typename C>
	inline const C* end(const impl::buffer<C>& sv)
	{
		return sv.end();
	}
...
namespace zst
{
	...
	struct Ok<void>
	{
		...
		friend struct Result;
		template <typename, typename>
		friend struct zpr::print_formatter;
	};
	...
	public:
		...
		Result(Result&& other)
		{
			...
			if(this->state == STATE_VAL)
				new(&this->val) T(static_cast<T&&>(other.val));
			if(this->state == STATE_ERR)
				new(&this->err) E(static_cast<E&&>(other.err));
		}
		...
		template <typename U,
			typename = std::enable_if_t<std::is_pointer_v<T> && std::is_pointer_v<U> &&
										std::is_base_of_v<std::remove_pointer_t<U>, std::remove_pointer_t<T>>>>
		operator Result<U, E>() const
		{
			using R = Result<U, E>;
			if(state == STATE_VAL)
				return R(typename R::tag_ok {}, this->val);
			if(state == STATE_ERR)
				return R(typename R::tag_err {}, this->err);
		...
		auto map(Fn&& fn) const -> Result<decltype(fn(detail::declval<T>())), E>
		{
			...
			{
				if(this->ok())
					return Res(typename Res::tag_ok {}, fn(this->val));
				else
					return Res(typename Res::tag_err {}, this->err);
			}
		}
		...
		Result<void, E> remove_value() const
		{
			...
			if(this->ok())
				return Res(typename Res::tag_ok {});
			else
				return Res(typename Res::tag_err {}, this->err);
		}
	...
	public:
		...
		auto map(Fn&& fn) const -> Result<decltype(fn()), E>
		{
			...
			if(this->ok())
				return Res(typename Res::tag_ok {}, fn());
			else
				return Res(typename Res::tag_err {}, this->err);
		}
		...
		Result<T, E> add_value(T&& value) const
		{
			...
			if(this->ok())
				return Res(typename Res::tag_ok {}, static_cast<T&&>(value));
			else
				return Res(typename Res::tag_err {}, this->err);
		}
